...
std::string save() {
    ...
    std::cout << "Enter the name of the file where you want to save the "
            "result (type 's' to see saved files, 'q' to quit to main menu): ";
    std::string filename;
    std::cin >> filename;
    while (filename == "s") {
    ...
    std::filesystem::path saving = std::filesystem::path(PROJECT_ROOT) 
                                       / "save" / "order_parameter" / filename;
    if (std::filesystem::exists(saving)) {
        std::cout << "Warning: file name '" << filename 
